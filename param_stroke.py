...
def stroke_renderer(curve_points: T.Tensor, locations: T.Tensor, colors: T.Tensor, widths: T.Tensor,
    ...
    canvas_with_nearest_Bs = curve_points[indices.flatten()].view(
        H, W, K, S, 2)
    ...
    # Compute distance from every pixel on canvas to each (among nearest ones)
    # line segment between points from curves
    indices_a = T.tensor([i for i in range(S - 1)], dtype=T.long).to(device)
    # start points of each line segment
    canvas_with_nearest_Bs_a = canvas_with_nearest_Bs[:, :, :, indices_a, :]
    indices_b = T.tensor([i for i in range(1, S)], dtype=T.long).to(device)
    # end points of each line segments
    canvas_with_nearest_Bs_b = canvas_with_nearest_Bs[:, :, :, indices_b, :]
    canvas_with_nearest_Bs_b_a = canvas_with_nearest_Bs_b - \
        canvas_with_nearest_Bs_a  # [H, W, N, S - 1, 2]
    # [H, W, K, S - 1, 2]
    P_full_canvas_with_nearest_Bs_a = P_full[:,
                                             :, None, None, :] - canvas_with_nearest_Bs_a
    ...
    # the first vector is the line segments. the second vector is the sample
    # points -> grid
    t = T.sum(canvas_with_nearest_Bs_b_a * P_full_canvas_with_nearest_Bs_a, dim=-1) / (
        T.sum(canvas_with_nearest_Bs_b_a ** 2, dim=-1) + 1e-8)

    # if t value is outside [0, 1], then the nearest point on the line does
    # not lie on the segment, so clip values of t
    t = T.clamp(t, 0., 1.)

    # compute closest points on each line segment, which are the projections
    # on each segment - [H, W, K, S - 1, 2]
    closest_points_on_each_line_segment = canvas_with_nearest_Bs_a + \
        t[..., None] * canvas_with_nearest_Bs_b_a

    # compute the distance from every pixel to the closest point on each line
    # segment - [H, W, K, S - 1]
    ...
    D_per_strokes = T.amin(
        dist_to_closest_point_on_line_segment,
        dim=-1)  # [H, W, K]
    D = T.amin(D_per_strokes, dim=-1)  # [H, W]

    # Finally render curves on a canvas to obtain image.
    I_NNs_B_ranking = F.softmax(
        100000. * (1.0 / (1e-8 + D_per_strokes)), dim=-1)  # [H, W, N]
    I_colors = T.einsum(
        'hwnf,hwn->hwf',
        canvas_with_nearest_Bs_colors,
        I_NNs_B_ranking)  # [H, W, 3]
    bs = T.einsum(
        'hwnf,hwn->hwf',
        canvas_with_nearest_Bs_bs,
        I_NNs_B_ranking)  # [H, W, 1]
...
class BrushStrokeRenderer(T.nn.Module):
    def __init__(self, canvas_height, canvas_width, num_strokes=5000, samples_per_curve=10, strokes_per_pixel=20,
        ...
        if content_img is not None:
            location, s, e, c, width, color = utils_brush.initialize_brushstrokes(content_img, num_strokes,
        ...
        e = e[..., ::-1]
        c = c[..., ::-1]
        self.curve_s = T.nn.Parameter(
            T.from_numpy(
                np.array(
                    s,
                    'float32')),
            requires_grad=True)
        self.curve_e = T.nn.Parameter(
            T.from_numpy(
                np.array(
                    e,
                    'float32')),
            requires_grad=True)
        self.curve_c = T.nn.Parameter(
            T.from_numpy(
                np.array(
                    c,
                    'float32')),
        ...
        self.location = T.nn.Parameter(
            T.from_numpy(
                np.array(
                    location,
                    'float32')),
            requires_grad=True)
        self.width = T.nn.Parameter(
            T.from_numpy(
                np.log(
                    np.minimum(
                        width,
