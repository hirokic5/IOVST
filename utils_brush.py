...
def clusters_to_strokes(segments, img, H, W, sec_scale=0.001, width_scale=1):
    ...
    for cluster_idx in range(num_clusters + 1):
        ...
        cluster_points = np.stack(
            (cluster_mask_nonzeros[0], cluster_mask_nonzeros[1]), axis=-1)
        try:
            convex_hull = ConvexHull(cluster_points)
        except BaseException:
            continue

        # find the two points (pixels) in the cluster that have the largest
        ...
        # compute the two intersection points of the line that goes orthogonal
        ...
        clusters_params['width'].append(
            np.sum((intersec_points[0] - intersec_points[1]) ** 2))
        ...
        clusters_params['stddev'].append(
            np.mean(np.std(img[cluster_mask], axis=0)))
    ...
    rel_num_pixels = np.clip(
        rel_num_pixels,
        rel_num_pix_quant[0],
        rel_num_pix_quant[1])
    cluster_width = np.clip(cluster_width, width_quant[0], width_quant[1])
    width = width_scale * \
        rel_num_pixels.reshape(-1, 1) * cluster_width.reshape(-1, 1)
    s, e, c = [x * sec_scale for x in [s, e, c]]

    location, s, e, c, width, color = [
        x.astype(
            np.float32) for x in [
            location, s, e, c, width, color]]
...
def initialize_brushstrokes(content_img, num_strokes, canvas_height,
                            canvas_width, sec_scale, width_scale, init='sp'):
    if init == 'random':
        # Brushstroke colors
