...
class Normalization(T.nn.Module):
    def __init__(self):
        ...
        self.register_buffer('kern', T.from_numpy(np.array(
            [[0, 0, 255], [0, 255, 0], [255, 0, 0]], 'float32')[:, :, None, None]))
        self.register_buffer('bias', T.from_numpy(
            np.array([-103.939, -116.779, -123.68], 'float32')))
...
class VGG19(T.nn.Sequential):
    ...
    def load_params(self, param_file):
        if not os.path.exists(param_file):
            ...
            utils_brush.download_file('https://github.com/ftokarev/tf-vgg-weights/raw/master/vgg19_weights_normalized.h5',
                                      param_file)
